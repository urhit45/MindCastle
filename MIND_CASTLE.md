Mind Castle
A personal operating system for focus, life, and mental clarity. "A second brain that actually understands you."
...
For many people, the challenge isn't a lack of attention. It's a lack of attention regulation.

## The Vision

Mind Castle is a local-first personal OS that helps you:
...
Local-first — Your sensitive thoughts live on your device, not a server.
Forgiveness over streaks — There is no streak to break. Every session is a fresh start.
One next task — No giant lists. See only the single smallest step to prevent paralysis.
Zero friction capture — Voice, text, or photo. Offload your brain in seconds.
...
Neurodivergent thinkers — People whose brains work differently from the standard mould.
...
Anyone who's felt failed by conventional productivity tools — regardless of diagnosis.
...
MindCastle shows you a copyable prompt to paste into your AI tool.
The AI reviews your conversation history and exports your projects, goals, and tasks as a structured JSON block.
You paste the response back into MindCastle.
MindCastle parses it, shows you a preview of the engines and artifacts it found, and imports them in one click.
...
Select your engines — Pick from 8 preset life-domain templates (multi-select grid).
Customise names — Rename or retitle each engine to match your life.
Castle rises — An animated celebration as your palace is constructed.
...
Paste any raw text — notes, tasks, journal entries, brain dumps.
MindCastle sends each line to the TinyNet classifier.
Lines are grouped by detected category and become engines, with each text chunk as an artifact.
Preview the structure, adjust if needed, and import.
...
Fitness Running Strength Music Guitar Learning AI Admin Finance Social Health Cooking Travel Work SideProject Design Reading Writing Mindfulness Household
...
PracticeForDuration · RepeatTask · IncreaseVolume · ScheduleFollowUp · AttachLink ReviewNotes · OutlineThreeBullets · BookAppointment · BuySupplies · CreateSubtasks SetReminder · LogReflection
...
The TinyNet model trains automatically from data/train.jsonl on first API startup. To regenerate training data from raw markdown notes:
...
- TinyNet on-device classifier
- Progress log with AI state detection
- Per-engine context (AI instructions)
- 3-path onboarding wizard (AI import, palace builder, text import)
- Local-only, zero-server, privacy-first
...
Many of us have brains that don't fit the standard mould — whether that's ADHD, autism, anxiety, or simply a non-linear way of thinking. Mind Castle is built for all of us. It needs:
...
Mind Castle is not a productivity app. It is a place your mind can come home to.
