...
from ..services.graph_learning import update_graph_edges_for_log
...
async def add_log(
    ...
    await session.flush()  # ensure timestamps/ids exist for graph updates
    await update_graph_edges_for_log(
        session,
        node_id=str(node.id),
        state=request.state,
        next_step=request.next_step,
